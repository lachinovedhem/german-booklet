...
const CategoryCard = ({ to, icon: Icon, label, count, countLabel, color }) => (
    <Link
        to={to}
...
const Home = () => {
    ...
    const categories = [
        { to: '/grammar', icon: FileText, label: t('nav_grammar'), count: counts.grammar, color: 'from-indigo-500 to-blue-500' },
        { to: '/nouns', icon: BookOpen, label: t('nav_nouns'), count: counts.nouns, color: 'from-blue-500 to-cyan-500' },
        { to: '/verbs', icon: Zap, label: t('nav_verbs'), count: counts.verbs, color: 'from-rose-500 to-orange-500' },
        { to: '/adjectives', icon: Star, label: t('nav_adjectives'), count: counts.adjectives, color: 'from-purple-500 to-pink-500' },
        { to: '/professions', icon: Briefcase, label: t('nav_professions'), count: counts.professions, color: 'from-amber-500 to-yellow-500' },
        { to: '/phrases', icon: MessageSquare, label: t('nav_phrases'), count: counts.phrases, color: 'from-indigo-500 to-purple-500' },
        { to: '/miscellaneous', icon: Grid, label: t('nav_misc'), count: counts.miscellaneous, color: 'from-emerald-500 to-teal-500' },
    ];

    const tipTitle = lang === 'en' ? (randomTip?.titleEn || randomTip?.title) : randomTip?.title;
    const tipDesc = lang === 'en' ? (randomTip?.descriptionEn || randomTip?.description) : randomTip?.description;
        ...
        <div className="space-y-6">
            ...
            <section className="relative overflow-hidden rounded-3xl bg-slate-900 p-8 text-white shadow-xl">
                <div className="relative z-10 max-w-2xl">
                    <h1 className="text-3xl font-black mb-2 tracking-tight">
                        {t('home_hero_title_1')} <span className="text-primary-light">{t('home_hero_title_hl')}</span> {t('home_hero_title_2')}
                    </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Link to="/start" className="glass-card p-5 rounded-2xl flex items-center gap-4 hover:scale-[1.02] transition-all duration-200 border-l-4 border-emerald-500">
                    ...
                    <div>
                        <h3 className="font-black text-slate-800">{t('home_beginner_title')}</h3>
                        <p className="text-xs text-slate-500">{t('home_beginner_desc')}</p>
                    </div>
                </Link>
                <Link to="/practice" className="glass-card p-5 rounded-2xl flex items-center gap-4 hover:scale-[1.02] transition-all duration-200 border-l-4 border-primary">
                    ...
                    <div>
                        <h3 className="font-black text-slate-800">{t('home_practice_title')}</h3>
                        <p className="text-xs text-slate-500">{t('home_practice_desc')}</p>
                    </div>
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {categories.map((cat) => (
                    <CategoryCard key={cat.to} {...cat} countLabel={t('home_count_suffix')} />
                ))}
            </div>
            ...
            {randomTip && (
                <div className="glass-card p-4 rounded-2xl border-l-4 border-primary">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h4 className="font-bold text-slate-800 text-sm mb-1">{tipTitle}</h4>
