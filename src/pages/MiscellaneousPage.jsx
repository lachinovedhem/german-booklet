...
const MiscellaneousPage = () => {
    ...
    const types = useMemo(() => {
        const set = new Set(miscData.map(item => item.type));
    ...
    const filteredData = useMemo(() => {
        ...
        return miscData.filter(item => {
            const matchesSearch =
                ...
                (item.trans && item.trans.toLowerCase().includes(q)) ||
            ...
            <div className="sticky top-16 md:top-0 z-20 -mx-4 md:-mx-6 px-4 md:px-6 py-4 bg-slate-50/95 backdrop-blur-md border-b border-slate-200/50">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-4">
                    <div className="space-y-2">
                        <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter">{t('misc_title')}</h2>
                        <p className="text-slate-500 font-medium max-w-md hidden md:block">{t('misc_sub')}</p>
                    </div>
                        ...
                        >
                            {types.map(ty => (
                                <option key={ty} value={ty}>
                                    {ty === 'all' ? t('all') : tCat('miscTypes', ty)}
                                </option>
                            ))}
                        </select>
                ...
                <div className="relative">
                    ...
                    <SearchBar value={search} onChange={setSearch} placeholder={t('misc_search')} />
                </div>
            </div>
